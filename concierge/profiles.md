...
| `reference/observation-integrity.md` | `reference/observation-integrity.md` | kit-template — the observation-integrity contract (the instrument test, the three failure shapes, the five runtime rules, the per-gate entailment requirement); SINGLE normative home (standing-rules carries only a pointer); index row same-change (rule 13) |
