...
If the session touched runtime-facing code, gather minimal verification (a `{{BUILD_CMD}}` / `{{LINT_CMD}}` / `{{TEST_CMD}}` result you already have; a `{{CODE_INTEL_TOOL}}` reachability output for a wiring claim) — only what's relevant to the chunk. Don't over-fetch.
