...
# Status — {{PROJECT_NAME}} · <one-line current headline> · <YYYY-MM-DD>
...
## Branch / working tree
- Branch `<branch>` (base: `{{DEFAULT_BRANCH}}`). <pushed? ahead-by-N? never-stage list, if any.>
- HEAD `<short-sha>` — <one-line description of the tip commit.>

## Build / test state
- Gates: `{{BUILD_CMD}}` · `{{TEST_CMD}}` · `{{LINT_CMD}}` · `{{FMT_CMD}}` — <last-run result / clean
  or the failing gate.>
